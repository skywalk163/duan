...
用法：
  duan7 <源文件.duan>            # 编译并运行
  duan7 compile <源文件.duan>     # 仅编译，输出到 stdout
  duan7 compile <源文件.duan> -o <输出.py>  # 编译到文件
  duan7 compile --c <源文件.duan>  # 编译为 C 代码
  duan7 compile --native <源文件.duan>  # 编译为原生可执行文件
  duan7 run <源文件.duan>         # 编译并运行
  duan7 --debug <源文件.duan>     # 开启调试模式运行
  duan7 --debug-file <文件.log> <源文件.duan>  # 调试日志输出到文件
  duan7 --demo                    # 运行综合演示（demo_l7.duan）
  duan7 --test                    # 运行内置测试
  duan7 --help                    # 显示帮助
"""
...
def 交互式REPL():
    """启动简单的交互式 REPL"""
    编译, ns = _加载编译器()
    print("段言 Level 7 REPL (输入 'exit()' 退出)")
    print("输入段言代码，按 Ctrl+Z 或输入 exit() 退出")
    print("-" * 40)
    buffer = ""
    while True:
        try:
...
def 主函数():
    ...
    if show_demo:
        # 查找 demo_l7.duan 文件
        demo_paths = [
            os.path.join(_SCRIPT_DIR, 'demo_l7.duan'),
            os.path.join(os.getcwd(), 'demo_l7.duan'),
        ]
        if getattr(sys, 'frozen', False):
            _MEIPASS = getattr(sys, '_MEIPASS', None)
            if _MEIPASS:
                demo_paths.insert(0, os.path.join(_MEIPASS, 'demo_l7.duan'))
        ...
        if not demo_path:
            print("[错误] 找不到演示文件: demo_l7.duan", file=sys.stderr)
            sys.exit(1)
        
        with open(demo_path, 'r', encoding='utf-8') as f:
            源代码 = f.read()
        print("=" * 50)
        print("段言 Level 7 综合演示（含类型注解）")
        print("=" * 50)
...
def _print_help():
    """打印帮助信息"""
    print("段言（Duan）Level 7 编译器 - 类型注解 + P3 增强功能")
    print()
    print("用法:")
    print("  python duan7.py <文件.duan>              编译并运行（默认）")
    print("  python duan7.py compile <文件.duan>       仅编译，输出到 stdout")
    print("  python duan7.py compile <文件.duan> -o <输出.py>  编译到文件")
    print("  python duan7.py compile --c <文件.duan>   编译为 C 代码")
    print("  python duan7.py compile --native <文件.duan>  编译为原生可执行文件")
    print("  python duan7.py run <文件.duan>           编译并运行")
    print("  python duan7.py --debug <文件.duan>       开启调试模式运行")
    print("  python duan7.py --debug-file <文件.log> <文件.duan>  调试日志输出到文件")
    print("  python duan7.py --demo                    运行综合演示")
    print("  python duan7.py --test                    运行内置测试")
    print("  python duan7.py --help                    显示帮助信息")
    print()
    print("选项:")
    print("  --debug               开启调试模式 (stdout)")
    print("  --debug-file FILE     调试日志输出到文件")
    print("  --demo                运行综合演示 (demo_l7.duan)")
    print("  --test                运行内置测试")
