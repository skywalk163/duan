# -*- coding: utf-8 -*-
"""
段言代码迁移工具 (Duan Migrator) v1.1

统一的代码迁移入口，支持 v3.3 → v4.0 → v5.x 的语法转换。

用法:
    from migration.duan_migrator import DuanMigrator

    migrator = DuanMigrator()
    # 自动检测并迁移
...
命令行:
    python -m src.migration.duan_migrator detect file.duan     # 检测语法版本
    python -m src.migration.duan_migrator v33-v40 file.duan    # v3.3 → v4.0
    python -m src.migration.duan_migrator v40-v50 file.duan    # v4.0 → v5.x
    python -m src.migration.duan_migrator auto file.duan       # 自动检测并迁移
    python -m src.migration.duan_migrator auto --preview file.duan  # 预览
"""
