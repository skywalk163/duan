        ...
        for i, line in enumerate(self._source_lines, 1):
            ...
            for ident in identifiers:
                ...
                if ident in ('设', '定义', '段落', '段', '函数', '函', '导入', '引', '如果', '若', '当', '遍历', '遍',
                             ...
                             '抛出', '抛', '最终', '终', '匹配', '配', '情况', '真', '假', '空', '是', '且', '或', '非'):
                    continue
                # 如果它是定义行的一部分，跳过
...
def lint_file(filepath: str, linter: DuanLinter) -> int:
    """检查单个文件"""
...
def lint_directory(directory: str, linter: DuanLinter) -> int:
    """检查目录"""
    ...
    print(f"\n总计: {len(duan_files)} 个文件, {total_errors} 个错误")
