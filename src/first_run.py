...
def get_config_dir() -> str:
    ...
    优先使用环境变量 DUAN_CONFIG_DIR，否则使用 ~/.duan
    """
    env_dir = os.environ.get('DUAN_CONFIG_DIR')
    if env_dir:
        return env_dir
    return os.path.join(os.path.expanduser("~"), ".duan")
...
WELCOME_BANNER = f"""
{C['title']}╔══════════════════════════════════════════════════════════╗{C['reset']}
{C['title']}║             🀄  段言 (DuanLang)  v6.0.0  🀄             ║{C['reset']}
{C['title']}║                                                          ║{C['reset']}
...
MENU_OPTIONS = {
    '1': {
        'label': '快速入门（5分钟交互式体验）',
        'description': '通过 5 个互动练习快速掌握段言基础语法',
    },
    '2': {
        'label': '打开文档站',
        'description': '在浏览器中打开段言官方文档',
    },
    '3': {
        'label': '直接进入 REPL',
...
DOCS_URL = "https://github.com/skywalk163/duan/tree/main/docs"
...
def run_interactive_tutorial() -> bool:
    ...
    try:
        ...
        print(f"  {C['tip']}请确保段言已正确安装。{C['reset']}")
        return False
...
def run_welcome() -> str:
    ...
    print(f"  {C['highlight']}欢迎使用段言（DuanLang）！{C['reset']}")
    print(f"  {C['dim']}段言是一门用中文自然语言编程的语言，{C['reset']}")
...
def main():
    """命令行入口"""
    import argparse
    parser = argparse.ArgumentParser(description='段言首次运行引导')
    parser.add_argument('--reset', action='store_true', help='重置首次运行标记')
    args = parser.parse_args()

    if args.reset:
        reset_first_run_flag()
        print("首次运行标记已重置。下次运行 duan 时将显示欢迎引导。")
        return
