"""
段言（Duan）编程语言 - 社区反馈收集器

提供 `duan feedback` CLI 命令，用于收集用户反馈。

功能：
  - duan feedback              交互式收集反馈（评分、评论、分类）
  - duan feedback --list       查看本地反馈记录
  - duan feedback --export     导出反馈为 JSON
  - duan feedback --send       提交本地反馈到 GitHub Issues

反馈数据存储位置：
  Windows: %USERPROFILE%\\.duan\\feedback\\
  macOS/Linux: ~/.duan/feedback/
"""
...
def get_feedback_dir() -> Path:
    """获取反馈数据存储目录"""
    env_dir = os.environ.get("DUAN_CONFIG_DIR")
    if env_dir:
        base = Path(env_dir)
    else:
        base = Path.home() / ".duan"
...
def send_feedback(record: Dict[str, Any]) -> bool:
    ...
    if not token:
        ...
        print("[提示] 也可以直接手动提交：https://github.com/skywalk163/duan/issues/new", file=sys.stderr)
        return False
...
> 此 Issue 由 `duan feedback --send` 自动创建
"""
    ...
    req = urllib.request.Request(
        "https://api.github.com/repos/skywalk163/duan/issues",
        data=data,
        headers={
...
def collect_feedback_interactive() -> Optional[Dict[str, Any]]:
    ...
    print("=" * 50)
    print()
    print("感谢您使用段言！您的反馈是我们改进的重要依据。")
    print()

    # 评分
    rating = _prompt_int("请为段言评分（1-5，5 为最高）: ", 1, 5)
    ...
    print(f"[提示] 运行 `duan feedback --send` 提交到 GitHub Issues")
    print()
...
def setup_feedback_subparser(subparsers) -> None:
    ...
    feedback_parser = subparsers.add_parser(
        "feedback",
        help="收集和查看用户反馈",
        description="段言反馈收集工具 - 收集、查看、导出和提交用户反馈",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="段言（Duan）反馈收集工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例：
  duan feedback              交互式收集反馈
  duan feedback --list       查看本地反馈
  duan feedback --export     导出反馈为 JSON
  duan feedback --send       提交到 GitHub Issues
  duan feedback --export ./my_feedback.json  导出到指定文件
        """,
    )
    parser.add_argument(
        "--list", "-l",
