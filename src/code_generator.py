...
class PythonCodeGenerator:
    ...
    def generate(self, module: Module) -> str:
        ...
        self._add_line("for _duan_n, _duan_f in [")
        ...
        self._add_line("]:")
        self._add_line("    if not hasattr(_duan_builtin, _duan_n):")
        self._add_line("        setattr(_duan_builtin, _duan_n, _duan_f)")
        self._add_line("if (not _duan_stdlib) or (not os.path.isdir(_duan_stdlib or '')):")
        self._add_line("    try:")
        self._add_line("        import types as _duan_types")
        self._add_line("        _duan_fs = _duan_types.ModuleType('文件系统')")
        self._add_line("        for _duan_fn in ('读取文件', '写入文件', '追加文件', '文件存在', '删除文件', '复制文件', '移动文件', '创建目录', '删除目录', '目录存在', '路径连接', '当前工作目录', '读取行'):")
        self._add_line("            if hasattr(_duan_builtin, _duan_fn):")
        self._add_line("                setattr(_duan_fs, _duan_fn, getattr(_duan_builtin, _duan_fn))")
        self._add_line("        sys.modules.setdefault('文件系统', _duan_fs)")
        self._add_line("    except Exception:")
        self._add_line("        pass")
