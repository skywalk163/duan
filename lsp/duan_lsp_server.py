...
def _translate_error(msg):
    """翻译错误消息为中文"""
    lower = msg.lower()
    for eng, zh in _duan_error_zh.items():
        if eng in lower:
            return zh
    return msg
...
def 获取补全(行, 字符位置):
    ...
    _duan_builtin.列表追加(关键词列表, "设")
    _duan_builtin.列表追加(关键词列表, "若")
    _duan_builtin.列表追加(关键词列表, "否")
    _duan_builtin.列表追加(关键词列表, "当")
    _duan_builtin.列表追加(关键词列表, "遍")
    _duan_builtin.列表追加(关键词列表, "段")
    _duan_builtin.列表追加(关键词列表, "返")
    _duan_builtin.列表追加(关键词列表, "跳")
    _duan_builtin.列表追加(关键词列表, "过")
    _duan_builtin.列表追加(关键词列表, "类")
    _duan_builtin.列表追加(关键词列表, "承")
    _duan_builtin.列表追加(关键词列表, "接")
    _duan_builtin.列表追加(关键词列表, "配")
    _duan_builtin.列表追加(关键词列表, "试")
    _duan_builtin.列表追加(关键词列表, "捕")
    _duan_builtin.列表追加(关键词列表, "抛")
    _duan_builtin.列表追加(关键词列表, "终")
    _duan_builtin.列表追加(关键词列表, "导")
    _duan_builtin.列表追加(关键词列表, "出")
    _duan_builtin.列表追加(关键词列表, "自")
    _duan_builtin.列表追加(关键词列表, "之")
    _duan_builtin.列表追加(关键词列表, "并")
    _duan_builtin.列表追加(关键词列表, "且")
    _duan_builtin.列表追加(关键词列表, "或")
    _duan_builtin.列表追加(关键词列表, "非")
    _duan_builtin.列表追加(关键词列表, "真")
    _duan_builtin.列表追加(关键词列表, "假")
    _duan_builtin.列表追加(关键词列表, "空")
    _duan_builtin.列表追加(关键词列表, "定义")
    _duan_builtin.列表追加(关键词列表, "如果")
    _duan_builtin.列表追加(关键词列表, "那么")
    _duan_builtin.列表追加(关键词列表, "否则")
    _duan_builtin.列表追加(关键词列表, "否则若")
    _duan_builtin.列表追加(关键词列表, "遍历")
    _duan_builtin.列表追加(关键词列表, "跳出")
    _duan_builtin.列表追加(关键词列表, "跳过")
    _duan_builtin.列表追加(关键词列表, "返回")
    _duan_builtin.列表追加(关键词列表, "段落")
    _duan_builtin.列表追加(关键词列表, "函数")
    _duan_builtin.列表追加(关键词列表, "导入")
    _duan_builtin.列表追加(关键词列表, "导出")
    _duan_builtin.列表追加(关键词列表, "从")
    _duan_builtin.列表追加(关键词列表, "打印")
    _duan_builtin.列表追加(关键词列表, "输出")
    ...
    _duan_builtin.列表追加(关键词列表, "加")
    _duan_builtin.列表追加(关键词列表, "减")
    _duan_builtin.列表追加(关键词列表, "乘")
    _duan_builtin.列表追加(关键词列表, "除")
    _duan_builtin.列表追加(关键词列表, "等于")
    _duan_builtin.列表追加(关键词列表, "大于")
    _duan_builtin.列表追加(关键词列表, "小于")
    _duan_builtin.列表追加(关键词列表, "转串")
    _duan_builtin.列表追加(关键词列表, "转整数")
    _duan_builtin.列表追加(关键词列表, "转浮点")
    _duan_builtin.列表追加(关键词列表, "新建")
    _duan_builtin.列表追加(关键词列表, "尝试")
    _duan_builtin.列表追加(关键词列表, "捕获")
    _duan_builtin.列表追加(关键词列表, "抛出")
    _duan_builtin.列表追加(关键词列表, "最终")
    _duan_builtin.列表追加(关键词列表, "匹配")
    _duan_builtin.列表追加(关键词列表, "情况")
    _duan_builtin.列表追加(关键词列表, "接口")
    _duan_builtin.列表追加(关键词列表, "异步")
    _duan_builtin.列表追加(关键词列表, "等待")
    i = 0
    while (i < _duan_builtin.列表长度(关键词列表)):
        关键词 = _duan_builtin.列表获取(关键词列表, i)
        项 = _duan_builtin.字典创建()
        _duan_builtin.字典设置(项, "label", 关键词)
        _duan_builtin.字典设置(项, "kind", 14)
        # 添加中文文档
        文档 = _duan_kw_docs.get(关键词, "")
        if 文档:
            _duan_builtin.字典设置(项, "documentation", 文档)
        _duan_builtin.列表追加(结果, 项)
        i = (i + 1)
    return 结果

def 主():
    _duan_builtin.打印错误("段言 LSP 服务器启动（支持中文补全和悬停提示）")
    运行中 = 1
    while (运行中 == 1):
        请求 = 读取LSP消息()
        if (请求 == None):
            ...
            if (方法 == "initialize"):
                ...
                _duan_builtin.字典设置(服务器信息, "version", "2.0.0")
                ...
                if (方法 == "textDocument/didChange"):
                    ...
                    诊断列表 = 获取诊断(文本)
                    # 翻译诊断消息为中文
                    翻译后列表 = _duan_builtin.列表创建()
                    di = 0
                    while (di < _duan_builtin.列表长度(诊断列表)):
                        诊断 = _duan_builtin.列表获取(诊断列表, di)
                        原消息 = _duan_builtin.字典获取(诊断, "message")
                        翻译消息 = _translate_error(原消息)
                        _duan_builtin.字典设置(诊断, "message", 翻译消息)
                        _duan_builtin.列表追加(翻译后列表, 诊断)
                        di = (di + 1)
                    ...
                    _duan_builtin.字典设置(pub_params, "diagnostics", 翻译后列表)
                    ...
                    if (方法 == "textDocument/didOpen"):
                        ...
                        if (方法 == "textDocument/completion"):
                            ...
                            if (方法 == "textDocument/hover"):
                                # 处理悬停请求
                                参数 = _duan_builtin.字典获取(请求, "params", _duan_builtin.字典创建())
                                文本文档 = _duan_builtin.字典获取(参数, "textDocument", _duan_builtin.字典创建())
                                位置 = _duan_builtin.字典获取(参数, "position", _duan_builtin.字典创建())
                                悬停结果 = _duan_builtin.字典创建()
                                _duan_builtin.字典设置(悬停结果, "contents", _duan_builtin.字典创建())
                                _duan_builtin.字典设置(_duan_builtin.字典获取(悬停结果, "contents"), "kind", "markdown")
                                _duan_builtin.字典设置(_duan_builtin.字典获取(悬停结果, "contents"), "value", "段言 LSP 服务器 v2.0.0\n\n支持中文关键字补全和中文文档悬停提示。")
                                发送LSP响应(请求id, 悬停结果)
                            else:
                                if (方法 == "shutdown"):
                                    发送LSP响应(请求id, None)
