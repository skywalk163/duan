...
时出现：
  - lists 失败：AttributeError: module '_duan_builtin' has no attribute '列表排序'
...
修复（src/code_generator.py、src/code_generator_unified.py 的 _duan_builtin 定义之后）
...
def _require_compiler():
    ...
    try:
        ...
        return DuanCompiler, UnifiedCodeGenerator
...
class TestMissingStdlibSrcBackendE2E(unittest.TestCase):
    ...
    def _run_duan(self, root, source):
