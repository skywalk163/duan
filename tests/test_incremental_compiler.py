...
class TestDependencyGraph:
    ...
    def test_add_module(self):
        ...
        graph.add_module('main', '/path/main.duan', ['utils', 'io'])
        graph.add_module('utils', '/path/utils.duan', [])
        graph.add_module('io', '/path/io.duan', [])
    ...
    def test_get_dependents(self):
        ...
        graph = DependencyGraph()
        graph.add_module('main', '/path/main.duan', ['utils', 'io'])
        graph.add_module('app', '/path/app.duan', ['utils'])
        graph.add_module('utils', '/path/utils.duan', [])
    ...
    def test_cascade_affected(self):
        ...
        graph = DependencyGraph()
        # A -> B -> C
        graph.add_module('A', '/path/a.duan', ['B'])
        graph.add_module('B', '/path/b.duan', ['C'])
        graph.add_module('C', '/path/c.duan', [])
    ...
    def test_cycle_detection(self):
        ...
        graph.add_module('A', '/path/a.duan', ['B'])
        graph.add_module('B', '/path/b.duan', ['C'])
        graph.add_module('C', '/path/c.duan', ['A'])  # 循环

        assert graph.has_cycle() is True

        # 无循环
        graph2 = DependencyGraph()
        graph2.add_module('A', '/path/a.duan', ['B'])
        graph2.add_module('B', '/path/b.duan', [])
    ...
    def test_topological_sort(self):
        ...
        graph = DependencyGraph()
        graph.add_module('A', '/path/a.duan', ['B', 'C'])
        graph.add_module('B', '/path/b.duan', ['C'])
        graph.add_module('C', '/path/c.duan', [])
    ...
    def test_module_path_mapping(self):
        ...
        graph = DependencyGraph()
        graph.add_module('main', '/path/main.duan', [])

        assert graph.get_module_path('main') == '/path/main.duan'
        assert graph.get_module_name('/path/main.duan') == 'main'
    ...
    def test_get_stats(self):
        ...
        graph = DependencyGraph()
        graph.add_module('A', '/path/a.duan', ['B'])
        graph.add_module('B', '/path/b.duan', [])
    ...
    def test_clear(self):
        ...
        graph = DependencyGraph()
        graph.add_module('A', '/path/a.duan', [])
...
class TestIncrementalCompiler:
    ...
    def test_compile_nonexistent_file(self, compiler):
        """测试编译不存在的文件"""
        result = compiler.compile('/nonexistent/file.duan')
        assert result['success'] is False
        assert 'error' in result
        assert '不存在' in result['error']

    @pytest.fixture
    def temp_duan_file(self, tmp_path):
        """创建临时 .duan 文件"""
        source_file = tmp_path / "test.duan"
        source_file.write_text('打印 "hello"。', encoding='utf-8')
        return str(source_file)

    def test_compile_file(self, compiler, temp_duan_file):
        """测试编译文件"""
        result = compiler.compile(temp_duan_file, force=True)
        # 可能编译失败（因为缺少 clang 等），但接口应该返回正确结构
        assert 'success' in result
        assert 'file_path' in result
        assert 'cached' in result
        assert result['file_path'] == os.path.abspath(temp_duan_file)

    def test_mark_dirty(self, compiler, temp_duan_file):
        """测试标记脏文件"""
        compiler.mark_dirty(temp_duan_file)
        assert os.path.abspath(temp_duan_file) in compiler._dirty_files
...
'''
    ...
    def test_find_duan_files(self, compiler, tmp_path):
        """测试查找 .duan 文件"""
        # 创建一些 .duan 文件
        (tmp_path / "a.duan").write_text("test", encoding='utf-8')
        (tmp_path / "b.duan").write_text("test", encoding='utf-8')
        sub = tmp_path / "sub"
        sub.mkdir()
        (sub / "c.duan").write_text("test", encoding='utf-8')

        files = compiler._find_duan_files(str(tmp_path))
        assert len(files) == 3
        assert all(f.endswith('.duan') for f in files)
    ...
    def test_dep_graph_builder(self, compiler, tmp_path):
        """测试依赖图构建"""
        # 创建模块文件
        utils = tmp_path / "utils.duan"
        utils.write_text('''段言工具模块
设 版本 为 1。
''', encoding='utf-8')

        main = tmp_path / "main.duan"
        main.write_text('''导入 "utils"
设 x 为 1。
''', encoding='utf-8')

        # 构建依赖图
        duan_files = [str(main), str(utils)]
        compiler._build_dependency_graph(duan_files)
