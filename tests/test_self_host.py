...
def _run_python_code(py_code: str, capture_output: bool = False):
    ...
    _duan_builtin.阶乘 = lambda n: __import__('math').factorial(n)
    _duan_builtin.平均数 = lambda data: sum(data) / len(data) if data else 0
    _duan_builtin.求和 = lambda data: sum(data)
    _duan_builtin.时间戳 = lambda: __import__('time').time()
    _duan_builtin.格式化时间 = lambda ts, fmt='%Y-%m-%d %H:%M:%S': __import__('time').strftime(fmt, __import__('time').localtime(ts))
    ...
    _duan_builtin.创建目录 = lambda path: __import__('os').makedirs(path, exist_ok=True)
    _duan_builtin.圆周率 = lambda: __import__('math').pi
    _duan_builtin.自然常数 = lambda: __import__('math').e
    _duan_builtin._读文件 = lambda path: open(path, 'r', encoding='utf-8').read()

    namespace = {
        '_duan_builtin': _duan_builtin,
