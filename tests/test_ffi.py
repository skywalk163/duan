...
def test_codegen_struct_as_param_type():
    ...
    # 验证运行时注册类型
    assert "_duan_ffi.注册类型('点', 点)" in result
...
def test_codegen_nested_struct():
    ...
    # 验证两个结构体都注册了类型
    assert "_duan_ffi.注册类型('点', 点)" in result
    assert "_duan_ffi.注册类型('线段', 线段)" in result
    print("  [OK] 嵌套结构体")
...
def test_codegen_union_as_param_type():
    ...
    assert "_duan_ffi.注册类型('数据', 数据)" in result
...
def test_codegen_funcptr_as_param_type():
    ...
    assert "_duan_ffi.注册类型('比较器', 比较器)" in result
...
def test_codegen_struct_array_usage():
    ...
    assert '_duan_ffi.创建数组(点, 5)' in result
    print("  [OK] 结构体数组")
