"""
测试 lsp/lsp_protocol.duan（段言 LSP 协议层）编译与协议行为

说明：原文件为模块级脚本（pytest 收集 0 个测试，属「虚假通过」），且依赖
DuanCompiler.compile 返回 'code'（API 已变更导致 KeyError）。已迁移为 pytest
测试：使用 src 后端编译 lsp_protocol.duan，并模拟 LSP initialize/shutdown
请求验证协议层真实工作。
"""
...
def _compile_lsp_protocol() -> str:
    """编译 lsp/lsp_protocol.duan，返回生成的 Python 代码"""
    project_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..')
    source_file = os.path.join(project_root, 'lsp', 'lsp_protocol.duan')
...
class TestLSPProtocol:
    ...
    def test_initialize与shutdown协议(self):
        ...
        assert init_resp['result']['serverInfo']['name'] == 'duan-lsp'
        # shutdown 响应应包含 id=2
