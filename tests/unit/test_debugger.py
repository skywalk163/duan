...
测试 DuanDebugger 核心功能：断点管理、单步模式、调用栈跟踪。
"""
...
from duan_debug import DuanDebugger, DebuggerContext, Breakpoint, StackFrame, create_debugger
...
class TestDuanDebugger(unittest.TestCase):
    """调试器核心测试"""
    ...
    def test_set_and_get_breakpoint(self):
        bp = self.debugger.set_breakpoint('test.duan', 10)
        self.assertEqual(bp.line, 10)
        found = self.debugger.get_breakpoint('test.duan', 10)
        self.assertIsNotNone(found)
        self.assertEqual(found.line, 10)

    def test_clear_breakpoint(self):
        self.debugger.set_breakpoint('test.duan', 10)
        self.assertTrue(self.debugger.clear_breakpoint('test.duan', 10))
        self.assertIsNone(self.debugger.get_breakpoint('test.duan', 10))

    def test_clear_nonexistent_breakpoint(self):
        self.assertFalse(self.debugger.clear_breakpoint('test.duan', 999))

    def test_list_breakpoints(self):
        self.debugger.set_breakpoint('a.duan', 5)
        self.debugger.set_breakpoint('a.duan', 10)
        self.debugger.set_breakpoint('b.duan', 3)
        all_bps = self.debugger.list_breakpoints()
        self.assertEqual(len(all_bps), 3)
        a_bps = self.debugger.list_breakpoints('a.duan')
        self.assertEqual(len(a_bps), 2)

    def test_check_breakpoint_hit(self):
        self.debugger.set_breakpoint('test.duan', 5)
        bp = self.debugger.check_breakpoint('test.duan', 5)
        self.assertIsNotNone(bp)
        self.assertEqual(bp.hit_count, 1)

    def test_check_breakpoint_miss(self):
        self.debugger.set_breakpoint('test.duan', 5)
        bp = self.debugger.check_breakpoint('test.duan', 10)
        self.assertIsNone(bp)

    def test_step_mode_default(self):
        self.assertEqual(self.debugger.step_mode, DuanDebugger.STEP_NONE)

    def test_set_step_over(self):
        self.debugger.set_step(DuanDebugger.STEP_OVER)
        self.assertEqual(self.debugger.step_mode, DuanDebugger.STEP_OVER)

    def test_set_step_into(self):
        self.debugger.set_step(DuanDebugger.STEP_INTO)
        self.assertEqual(self.debugger.step_mode, DuanDebugger.STEP_INTO)

    def test_should_stop_on_breakpoint(self):
        self.debugger.set_breakpoint('test.duan', 5)
        self.assertTrue(self.debugger.should_stop_here('test.duan', 5))

    def test_should_stop_on_step_into(self):
        self.debugger.set_step(DuanDebugger.STEP_INTO)
        self.assertTrue(self.debugger.should_stop_here('test.duan', 1))

    def test_should_not_stop_no_breakpoint_no_step(self):
        self.assertFalse(self.debugger.should_stop_here('test.duan', 1))
    ...
    def test_frame_callback(self):
        ...
        self.debugger.set_breakpoint('test.duan', 1)
        self.debugger.start()
        self.debugger.on_line_change('test.duan', 1, None)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0], ('test.duan', 1))
...
class TestDebuggerContext(unittest.TestCase):
    ...
    def test_trace_captures_lines(self):
        ...
        debugger.set_step(DuanDebugger.STEP_INTO)
        debugger.start()
...
class TestStackFrame(unittest.TestCase):
    ...
    def test_create_stack_frame(self):
        frame = StackFrame(name='测试', filename='test.duan', lineno=5)
        self.assertEqual(frame.name, '测试')
        self.assertEqual(frame.filename, 'test.duan')
