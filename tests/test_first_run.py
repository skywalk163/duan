...
def temp_config_dir():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        old_env = os.environ.get('DUAN_CONFIG_DIR')
        os.environ['DUAN_CONFIG_DIR'] = tmpdir
        yield tmpdir
        if old_env is not None:
            os.environ['DUAN_CONFIG_DIR'] = old_env
        else:
            del os.environ['DUAN_CONFIG_DIR']
...
class TestConfigDir:
    ...
    def test_get_config_dir_default(self):
        ...
        expected = os.path.join(os.path.expanduser("~"), ".duan")
        # 清除环境变量以测试默认值
        old_env = os.environ.pop('DUAN_CONFIG_DIR', None)
        try:
...
class TestWelcomeBanner:
    ...
    def test_print_welcome_banner(self, capsys):
        ...
        assert "段言" in captured.out
        assert "DuanLang" in captured.out
        assert "v6.0.0" in captured.out
...
class TestFirstRunFlow:
    ...
    def test_docs_url_defined(self):
        ...
        assert "duan" in DOCS_URL.lower() or "DuanLang" in DOCS_URL
