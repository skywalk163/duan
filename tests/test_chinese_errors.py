...
# =============================================================================
# D06: 测试 DuanError 自动添加修改指引
# =============================================================================

class TestDuanErrorWithHints:
    """测试 DuanError 自动集成修改指引"""

    def test_duan_error_with_hint_keyword(self):
        """测试 DuanError 自动匹配段言错误提示"""
        # 包含"设"的消息应自动匹配到"设"的指引
        error = DuanError('第 3 行的"设"关键字后缺少要定义的变量名', line=3)
        assert '💡' in str(error) or '提示' in str(error)

    def test_duan_error_without_hint(self):
        """测试 DuanError 在没有匹配关键字时不添加指引"""
        error = DuanError('常规错误', line=1)
        # 不应包含"设"的指引
        assert '设' not in str(error) or '缺少' not in str(error)

    def test_duan_error_with_explicit_hint(self):
        """测试 DuanError 显式传入的 hint 不被覆盖"""
        error = DuanError('第 5 行的"设"关键字后缺少变量名', line=5, hint='自定义提示')
        assert '自定义提示' in str(error)

    def test_lexer_error_inherits_hints(self):
        """测试 LexerError 继承 DuanError 的修改指引功能"""
    ...
    def test_semantic_error_inherits_hints(self):
        """测试 SemanticError 继承 DuanError 的修改指引功能"""
