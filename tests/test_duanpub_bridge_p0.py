"""
duanpub P0 桥接模块测试 - 文件系统 + JSON + CSV + 正则表达式 + 日期时间 + 数学运算 + 加密

验证 7 个 P0 包通过 duanpub 加载器导入后功能真实可用（导入→调用→结果验证全链路）。
对应计划 3.1.2：修复 duanpub 桥接测试虚假通过，每个桥接包有 ≥3 个真实功能测试。
"""
