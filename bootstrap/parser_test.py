...
__all__ = ['测试语法']
def 测试语法():
    甲 = _duan_builtin.字典创建()
    _duan_builtin.字典设置(甲, "类型", "变量声明")
    _duan_builtin.字典设置(甲, "名称", "乙")
    乙 = _duan_builtin.列表创建()
    _duan_builtin.列表追加(乙, "定义")
    _duan_builtin.列表追加(乙, "乙")
    _duan_builtin.列表追加(乙, "等于")
    print("语法分析完成")
    return 乙
