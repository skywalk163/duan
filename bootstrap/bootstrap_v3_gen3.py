# Generated by Duan bootstrap compiler
import sys
import types
...
def parse_expr(tokens, pos):
    esult = parse_or_expr(tokens, pos)
    return esult
...
def parse_or_expr(tokens, pos):
    ...
    while (p < _duan_builtin.列表长度(tokens)):
        ...
        if ((ttype == '关键字') and (tval == '或')):
            p += 1
            ight_result = parse_and_expr(tokens, p)
            ight = _duan_builtin.列表获取(ight_result, 0)
            p = _duan_builtin.列表获取(ight_result, 1)
            left = make_binary_op(tval, left, ight)
        else:
            break
    return [left, p]
...
def parse_and_expr(tokens, pos):
    ...
    while (p < _duan_builtin.列表长度(tokens)):
        ...
        if ((ttype == '关键字') and ((tval == '且') or (tval == '与'))):
            p += 1
            ight_result = parse_compare_expr(tokens, p)
            ight = _duan_builtin.列表获取(ight_result, 0)
            p = _duan_builtin.列表获取(ight_result, 1)
            left = make_binary_op(tval, left, ight)
        else:
    ...
    if (p < _duan_builtin.列表长度(tokens)):
        ...
        if (ttype == '关键字'):
            if ((((((tval == '等于') or (tval == '不等于')) or (tval == '大于')) or (tval == '小于')) or (tval == '大于等于')) or (tval == '小于等于')):
                p += 1
                ight_result = parse_add_expr(tokens, p)
                ight = _duan_builtin.列表获取(ight_result, 0)
                p = _duan_builtin.列表获取(ight_result, 1)
                left = make_binary_op(tval, left, ight)
    return [left, p]
...
def parse_add_expr(tokens, pos):
    ...
    while (p < _duan_builtin.列表长度(tokens)):
        ...
        if is_add_op:
            p += 1
            ight_result = parse_mul_expr(tokens, p)
            ight = _duan_builtin.列表获取(ight_result, 0)
            p = _duan_builtin.列表获取(ight_result, 1)
            left = make_binary_op(op_name, left, ight)
        else:
            break
    return [left, p]
...
def parse_mul_expr(tokens, pos):
    ...
    while (p < _duan_builtin.列表长度(tokens)):
        ...
        if is_mul_op:
            p += 1
            ight_result = parse_unary_expr(tokens, p)
            ight = _duan_builtin.列表获取(ight_result, 0)
            p = _duan_builtin.列表获取(ight_result, 1)
            left = make_binary_op(op_name, left, ight)
        else:
...
def parse_unary_expr(tokens, pos):
    ...
    esult = parse_primary(tokens, pos)
    return esult
...
def parse_primary(tokens, pos):
    ...
    if (ttype == '标识符'):
        ...
        if (p < _duan_builtin.列表长度(tokens)):
            ...
            if (_duan_builtin.字典获取(next_t, '种别') == '左括号'):
                ...
                p += 1
                if (p < _duan_builtin.列表长度(tokens)):
                    p_tok = _duan_builtin.列表获取(tokens, p)
                    if (_duan_builtin.字典获取(p_tok, '种别') != '右括号'):
                        while True:
                            ...
                            break
                if (p < _duan_builtin.列表长度(tokens)):
                    p = _duan_builtin.列表获取(tokens, p)
                    if (_duan_builtin.字典获取(p, '种别') == '右括号'):
                        p += 1
    ...
    if (ttype == '左书名号'):
        ...
        book = _duan_builtin.列表获取(tokens, (pos + 2))
...
def parse_return_stmt(tokens, pos):
    ...
    if (p < _duan_builtin.列表长度(tokens)):
        ...
        if ((ttype == '反缩进') or (ttype == '结束')):
            node = make_return(None)
            return [node, p]
        esult = parse_expr(tokens, p)
        node = make_return(_duan_builtin.列表获取(esult, 0))
        p = _duan_builtin.列表获取(esult, 1)
        return [node, p]
    node = make_return(None)
    return [node, p]
    ...
    if (p < _duan_builtin.列表长度(tokens)):
        ...
        if (_duan_builtin.字典获取(eq_tok, '种别') == '关键字'):
            if ((_duan_builtin.字典获取(eq_tok, '值') == '等于') or (_duan_builtin.字典获取(eq_tok, '值') == '为')):
                p += 1
    esult = parse_expr(tokens, p)
    node = make_var_decl(var_name, _duan_builtin.列表获取(esult, 0))
    return [node, _duan_builtin.列表获取(esult, 1)]
...
def parse_assign(tokens, pos):
    ...
    if (p < _duan_builtin.列表长度(tokens)):
        ...
        if ((_duan_builtin.字典获取(as_tok, '种别') == '关键字') and (_duan_builtin.字典获取(as_tok, '值') == '为')):
            p += 1
    esult = parse_expr(tokens, p)
    node = make_assign(var_name, _duan_builtin.列表获取(esult, 0))
    return [node, _duan_builtin.列表获取(esult, 1)]
...
def parse_compound_assign(tokens, pos, op):
    ...
    p = (pos + 1)
    esult = parse_expr(tokens, (p + 1))
    node = make_compound_assign(var_name, op, _duan_builtin.列表获取(esult, 0))
    return [node, _duan_builtin.列表获取(esult, 1)]
...
def parse_stmt(tokens, pos):
    ...
    if ((ttype == '关键字') and (tval == '段落')):
        esult = parse_paragraph_def(tokens, pos)
        return esult
    if ((ttype == '关键字') and (tval == '定义')):
        esult = parse_var_decl(tokens, pos)
        return esult
    if ((ttype == '关键字') and (tval == '设')):
        esult = parse_assign(tokens, pos)
        return esult
    if ((ttype == '关键字') and (tval == '返回')):
        esult = parse_return_stmt(tokens, pos)
        return esult
    if ((ttype == '关键字') and (tval == '如果')):
        esult = parse_if(tokens, pos)
        return esult
    if ((ttype == '关键字') and (tval == '当')):
        esult = parse_while(tokens, pos)
        return esult
    ...
    if (ttype == '标识符'):
        ...
        if ((pos + 1) < _duan_builtin.列表长度(tokens)):
            ...
            if ((((((nttype == '字符串') or (nttype == '数字')) or (nttype == '标识符')) or (nttype == '左中括号')) or (nttype == '左书名号')) or (nttype == '模板字符串')):
                ...
                node = make_func_call(name, args)
                return [node, p]
    esult = parse_expr(tokens, pos)
    node = make_expr_stmt(_duan_builtin.列表获取(esult, 0))
    return [node, _duan_builtin.列表获取(esult, 1)]
    ...
    while (pos < tok_count):
        ...
        if ((ttype == '关键字') and (tval == '从')):
            ...
            while (p < tok_count):
                ...
                p += 1
            continue
        esult = parse_stmt(tokens, p)
        stmt = _duan_builtin.列表获取(esult, 0)
        if (stmt != None):
            _duan_builtin.列表追加(stmts, stmt)
        pos = _duan_builtin.列表获取(esult, 1)
...
def get_output(state):
    ...
    while (i < _duan_builtin.列表长度(lines)):
        if (i > 0):
            esult = (esult + '\n')
        esult = (esult + _duan_builtin.列表获取(lines, i))
        i += 1
    return esult
...
def gen_expr(state, node):
    ...
    if (node_type == 'binary_op'):
        ...
        ight = gen_expr(state, _duan_builtin.列表获取(node, 3))
        py_op = op
        if ((op == '加') or (op == '+')):
            py_op = '+'
        elif ((op == '减') or (op == '-')):
