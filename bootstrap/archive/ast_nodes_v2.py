...
def 创建数字节点(值):
    节点 = _duan_builtin.列表创建()
    _duan_builtin.列表追加(节点, "数字")
    _duan_builtin.列表追加(节点, 值)
    return 节点

def 创建标识符节点(名字):
    节点 = _duan_builtin.列表创建()
    _duan_builtin.列表追加(节点, "标识符")
    _duan_builtin.列表追加(节点, 名字)
    return 节点

def 创建二元运算节点(操作符, 左侧, 右侧):
    节点 = _duan_builtin.列表创建()
    _duan_builtin.列表追加(节点, "二元运算")
    _duan_builtin.列表追加(节点, 操作符)
    _duan_builtin.列表追加(节点, 左侧)
    _duan_builtin.列表追加(节点, 右侧)
    return 节点

def 节点类型(节点):
    return _duan_builtin.列表获取(节点, 0)

def 节点值(节点):
    return _duan_builtin.列表获取(节点, 1)
