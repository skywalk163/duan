"""
Python → 段言 确定性转译器
将 Python AST 逐节点映射为段言 v3.2 源码

用法:
    python py2duan_transpiler.py input.py > output.duan
    echo 'print("hello")' | python py2duan_transpiler.py
...
class Py2DuanTranspiler:
    ...
    def _visit_Expr(self, node: ast.Expr):
        ...
        if isinstance(node.value, (ast.Yield, ast.YieldFrom)):
            ...
            return
        # Docstring (独立字符串常量表达式) -> 转为注释，段言不支持独立字符串语句
        if isinstance(node.value, ast.Constant) and isinstance(node.value.value, str):
