...
# -*- coding: utf-8 -*-
"""
Python 项目 -> 段言项目 批量转译器

将一个完整的 Python 项目（多文件、目录结构、import 关系）批量转译为段言项目。
基于单文件转译器 py2duan_transpiler.py 扩展。

用法:
    python pyproject2duan.py <源项目目录> [-o <输出目录>] [--dry-run] [--verbose]

示例:
    python pyproject2duan.py ./my_python_project -o ./my_duan_project
    python pyproject2duan.py ./my_python_project --dry-run --verbose
...
class ProjectTranspiler:
    ...
    def __init__(self, src_dir: str, out_dir: str,
        ...
        self.transpiler = Py2DuanTranspiler()
        ...
        self._suggestion_rules = {
            "Match": "match-case 模式匹配：段言不直接支持 match-case，转译为「匹配...情况」结构。",
            "AsyncFunctionDef": "async/await 异步函数：段言支持「异步 函数」语法，但运行时需要 asyncio 支持。",
            "decorator": "装饰器：段言使用「标注」关键字替代 Python 的 @decorator 语法。",
            "type_annotation": "类型注解：段言不直接支持类型注解，已忽略类型信息。",
            "Lambda": "lambda 匿名函数：段言支持「接收...返回...」匿名函数语法。",
            "Yield": "生成器：段言支持「产出」关键字替代 yield。",
            "NamedExpr": "海象运算符 (:=)：段言不支持海象运算符，请改用普通赋值。",
            "AnnAssign": "类型注解赋值：段言不支持类型注解，已忽略注解部分。",
            "chain_call": "链式调用：段言支持 obj.方法().属性 链式语法。",
            "fstring": "f-string：段言不直接支持 f-string，已转译为普通字符串拼接或格式化。",
        }
    ...
    def _transpile_file(self, src_file: Path, rel_path: Path):
        ...
        try:
            ...
            # import 路径重写：段言的"从 X.Y 导入 Z"不支持点号路径
            duan_code = self._rewrite_imports(duan_code, rel_path)

            # 段言解析器验证（返回详细结果）
            validate_result = self._validate_duan(duan_code)
    ...
    def _rewrite_imports(self, duan_code: str, rel_path: Path) -> str:
        """重写段言代码中的 import 语句，使其兼容段言解析器。

        段言的"从 X.Y 导入 Z"不支持模块路径中的点号。
        段言的"导入 X.Y"也不支持（点号被当语句分隔）。
    ...
    def _validate_duan(self, duan_code: str) -> dict:
        """用段言解析器验证 .duan 代码是否能成功解析
    ...
    def _generate_report(self):
        ...
        if total_issues > 0:
            ...
            for r in failed:
                ...
                if "TranspileError" in err_type:
                    active_suggestions.add(
                        "转译错误：某些 Python 语法段言不支持。请检查错误信息并手动修复对应代码。"
                    )
            ...
            lines.append("2. 对照原始 Python 源码和转译后的段言代码，找出差异")
            lines.append("3. 手动编辑 `.duan` 文件修复问题")
            lines.append("4. 运行 `python -m duan_parser_v3 <file>.duan` 验证修复")
            lines.append("")
        ...
        lines.append("")
        lines.append("# 方法 2: 使用段言 CLI 逐文件编译")
...
def _restore_imports(py_code: str, duan_source: str) -> str:
    ...
    段言编译器在解析时会丢弃注释行，所以 @import 注释不会出现在 .py 中。
...
# 段言内置函数名 -> Python 函数名映射
# 只包含段言代码生成器未正确映射回 Python 的函数
...
def _restore_builtins(py_code: str) -> str:
    """将段言代码生成器遗留的中文内置函数名替换回 Python 原名。

    段言代码生成器对部分内置函数（如 求和->sum）没有做反向映射，
...
def build_all():
    ...
    print(f"开始编译 {{len(duan_files)}} 个段言文件...")
    success = 0
    failed = 0
...
def main():
    parser = argparse.ArgumentParser(description="段言项目构建脚本")
...
def main():
    parser = argparse.ArgumentParser(
        prog="pyproject2duan",
        description="Python 项目 -> 段言项目 批量转译器",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python pyproject2duan.py ./my_project -o ./my_duan_project
  python pyproject2duan.py ./my_project --dry-run --verbose
  python pyproject2duan.py ./my_project -o ./output -v
        """,
    )
