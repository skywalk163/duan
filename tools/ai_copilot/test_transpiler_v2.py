...
from py2duan_transpiler import Py2DuanTranspiler, TranspileError
...
def verify_parse(name, duan_code):
    """验证段言代码能被 DuanParser 正确解析"""
    global PASS, FAIL
    try:
        parser = DuanParser()
        parser.parse(duan_code)
        PASS += 1
...
# 验证 _validate_duan 能正确检测有效/无效代码
try:
    from pyproject2duan import ProjectTranspiler

    # 创建临时目录测试
    tmpdir = tempfile.mkdtemp(prefix="duan_test_t6_")
...
print()
print("=" * 60)
print("段言解析验证: 验证转译结果能被 DuanParser 解析")
print("=" * 60)
