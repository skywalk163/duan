...
_HANDCRAFTED: List[tuple] = [
    # ── 变量与赋值 ────────────────────────────────────────────────
    ("变量", "x = 10", "设 x 为 10"),
    ("变量", "name = 'hello'", "定义 name 等于 \"hello\""),
    ...
    ("列表", "result = [x for x in lst if x > 0]",
     "设 result 为 []\n遍历 x 于 lst：\n    如果 x 大于 0：\n        result.追加(x)"),
    ("列表", "total = sum(lst)",
     "设 total 为 0\n遍历 x 于 lst：\n    设 total 为 total 加 x"),
    ("列表", "lst.sort()",
     "设 n 为 len(lst)\n遍历 i 于 0至n减2：\n    遍历 j 于 0至n减i减2：\n        如果 lst[j] 大于 lst[j 加 1]：\n            设 tmp 为 lst[j]\n            lst[j] = lst[j 加 1]\n            lst[j 加 1] = tmp"),
    ("列表", "lst.reverse()",
     "设 n 为 len(lst)\n设 i 为 0\n设 j 为 n 减 1\n当 i 小于 j：\n    设 tmp 为 lst[i]\n    lst[i] = lst[j]\n    lst[j] = tmp\n    i 加上 1\n    j 减去 1"),
    ("列表", "if 5 in lst:\n    print('found')",
     "设 found 为 假\n遍历 x 于 lst：\n    如果 x 等于 5：\n        设 found 为 真\n        跳出\n如果 found：\n    打印(\"found\")"),
    ("列表", "new_lst = lst[1:3]",
     "设 new_lst 为 []\n设 i 为 1\n当 i 小于 3：\n    new_lst.追加(lst[i])\n    i 加上 1"),
    ("列表", "count = lst.count(5)",
     "设 count 为 0\n遍历 x 于 lst：\n    如果 x 等于 5：\n        count 加上 1"),

    # ── 字符串 ────────────────────────────────────────────────────
    ("字符串", "s = 'hello' + ' world'", "定义 s 等于 \"hello\" 加 \" world\""),
    ("字符串", "n = len(s)", "设 n 为 len(s)"),
    ("字符串", "upper_s = s.upper()",
     "设 upper_s 为 字符串转大写(s)"),
    ("字符串", "if 'hello' in s:\n    print('found')",
     "如果 字符串包含(s, \"hello\")：\n    打印(\"found\")"),
    ("字符串", "parts = s.split(',')",
     "设 parts 为 字符串分割(s, \",\")"),
    ...
    # ── 补充：更多列表操作 ──────────────────────────────────────
    ("列表", "first = lst[0]\nlast = lst[-1]", "设 first 为 lst[0]\n设 last 为 lst[len(lst) 减 1]"),
    ("列表", "lst[2] = 99", "lst[2] = 99"),
    ("列表", "if len(lst) == 0:\n    print('empty')",
     "如果 len(lst) 等于 0：\n    打印(\"empty\")"),
    ("列表", "doubled = [x * 2 for x in lst]",
     "设 doubled 为 []\n遍历 x 于 lst：\n    doubled.追加(x 乘 2)"),
    ("列表", "evens = [x for x in lst if x % 2 == 0]",
     "设 evens 为 []\n遍历 x 于 lst：\n    如果 x 取余 2 等于 0：\n        evens.追加(x)"),
    ("列表", "merged = a + b",
     "设 merged 为 []\n遍历 x 于 a：\n    merged.追加(x)\n遍历 x 于 b：\n    merged.追加(x)"),
    ("列表", "lst.insert(0, 10)",
    ...
    # ── 补充：更多字符串 ────────────────────────────────────────
    ("字符串", "greeting = 'hi' + ' ' + name", "定义 greeting 等于 \"hi\" 加 \" \" 加 name"),
    ...
    ("暗坑", "for i in range(n):", "遍历 i 于 0至n减1："),
    ("暗坑", "class Foo:", "类 Foo："),
    ("暗坑", "try:", "尝试："),
    ("暗坑", "except:", "捕获 异常："),
