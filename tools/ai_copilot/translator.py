...
# Python → 段言 关键字映射
PYTHON_TO_DUAN_KEYWORDS: Dict[str, str] = {
    # 控制流
    'if': '如果',
    'elif': '否则如果',
    'else': '否则',
...
# 段言 → Python 关键字映射
DUAN_TO_PYTHON_KEYWORDS: Dict[str, str] = {
    # 控制流
    '如果': 'if',
    '否则如果': 'elif',
    '否则若': 'elif',
...
# Python 内置函数 → 段言 映射
PYTHON_TO_DUAN_BUILTIN: Dict[str, str] = {
    'print': '打印',
    'input': '读取',
    'len': '长度',
    'type': '类型',
...
# Python 异常名 → 段言 映射
PYTHON_TO_DUAN_EXCEPTION: Dict[str, str] = {
    'Exception': '异常',
    'ValueError': '值错误',
    'TypeError': '类型错误',
    'KeyError': '键错误',
...
class PythonToDuanTranslator:
    ...
    def _get_transpiler(self) -> Any:
        ...
        Returns:
            Py2DuanTranspiler 实例

        Raises:
            ImportError: 如果无法加载 py2duan_transpiler 模块
        """
    ...
    def translate_with_ast(self, python_code: str) -> Tuple[str, ast.AST]:
        """将 Python 源码翻译为段言代码，同时返回 AST

        Args:
            python_code: Python 源码字符串

        Returns:
            (翻译后的段言代码, Python AST 树)
        """
        tree = ast.parse(python_code)
    ...
    def analyze_features(self, python_code: str) -> Dict[str, int]:
        ...
        """
        try:
            from py2duan_transpiler import FeatureUsageCollector
            ...
            for file in files:
                ...
                try:
                    ...
                    if direction == 'to-duan':
                        ...
                        from .translator import duan_to_python
                        result = duan_to_python(duan_code=code)
    ...
    def translate_with_style(self, python_code: str, style: str = 'compact') -> str:
        ...
        if style == 'compact':
            ...
            compact_lines = []
            for line in lines:
                if line.strip().startswith('#') and not line.strip().startswith('# 段言'):
                    continue
            ...
            edu_lines = []
            edu_lines.append("# =============================================")
            edu_lines.append("# 段言 (DuanLang) 教学翻译")
    ...
    def roundtrip_verify(self, python_code: str) -> Dict[str, Any]:
        ...
        try:
            # Python → 段言
            duan_code = self.translate(python_code)
            result["duan"] = duan_code
        except Exception as e:
            result["errors"].append(f"Python→段言 翻译失败: {e}")
            return result

        try:
            # 段言 → Python
            from .translator import duan_to_python
            roundtrip_code = duan_to_python(duan_code=duan_code)
            result["roundtrip"] = roundtrip_code
        except Exception as e:
            result["errors"].append(f"段言→Python 翻译失败: {e}")
            return result
...
class DuanToPythonTranslator:
    ...
    # 关键字映射表（类级别，用于直接查询）
    keyword_map: Dict[str, str] = DUAN_TO_PYTHON_KEYWORDS
    ...
    def get_reverse_keywords(self) -> Dict[str, str]:
        """获取 Python→段言 反向关键字映射表

        Returns:
            反向关键字映射字典（Python 关键字 → 段言关键字）
        """
        reverse = {}
    ...
    def translate_with_style(self, duan_code: str, style: str = 'compact') -> str:
        """根据指定风格翻译段言代码为 Python 代码
        ...
        Returns:
            翻译后的 Python 代码字符串
        """
        base_result = self.translate(duan_code)
    ...
    def roundtrip_verify(self, duan_code: str) -> Dict[str, Any]:
        """验证段言→Python→段言 往返翻译的准确性
        ...
        try:
            python_code = self.translate(duan_code)
        ...
        try:
            from .translator import python_to_duan
            roundtrip_code = python_to_duan(python_code=python_code)
            result["roundtrip"] = roundtrip_code
        except Exception as e:
            result["errors"].append(f"Python→段言 翻译失败: {e}")
            return result
...
def python_to_duan(
    ...
    Returns:
        如果 return_ast 为 False，返回翻译后的段言代码字符串
        如果 return_ast 为 True，返回 (段言代码, AST) 元组
...
def duan_to_python(
    ...
    Raises:
        ValueError: 如果 duan_code 和 file_path 都为空
    """
    translator = DuanToPythonTranslator()
    if file_path and not duan_code:
        ...
        raise ValueError("必须提供 duan_code 或 file_path")
