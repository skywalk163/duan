...
扫描 duanpub/packages/ 下所有包，提取元数据，生成结构化索引文件 __index__.py。
可复用：duanpub 包增删后重新运行此脚本即可更新索引。

用法:
    python tools/gen_duanpub_index.py [--duanpub-path PATH] [--output PATH]

默认:
    --duanpub-path: C:/dumatework/duanpub
    --output:       stdlib/duanpub/__index__.py
"""
...
def extract_functions(duan_path: str) -> list:
    """从源.duan 中提取公开函数名（跳过 _内部 函数）"""
    funcs = []
    if not os.path.exists(duan_path):
        return funcs
...
def extract_ffi_count(duan_path: str) -> int:
    """统计 FFI 外部函数声明数量"""
    if not os.path.exists(duan_path):
        return 0
    count = 0
...
def scan_packages(duanpub_root: str) -> list:
    """扫描所有包，返回结构化元数据列表"""
    packages_dir = Path(duanpub_root) / 'packages'
...
def generate_index(packages: list, duanpub_root: str) -> str:
    """生成 __index__.py 内容"""
    lines = [
        '"""',
        'duanpub 包索引 (自动生成)',
        '',
        f'包总数: {len(packages)}',
        f'数据源: {duanpub_root}/packages/',
        f'生成工具: tools/gen_duanpub_index.py',
        '',
        '此文件由脚本自动生成，请勿手动编辑。',
        '如需更新索引，运行: python tools/gen_duanpub_index.py',
        '"""',
        '',
        '# 每个包的元数据',
        'PACKAGES = {',
    ...
    lines.append('# 导入名 -> duanpub 包名映射')
    lines.append('# 段言代码可用 "导入 标准文件系统" 或 "导入 文件系统" 访问')
...
def main():
    parser = argparse.ArgumentParser(description='duanpub 包索引生成器')
    parser.add_argument('--duanpub-path', default='C:/dumatework/duanpub',
                        help='duanpub 根目录路径')
    parser.add_argument('--output', default=None,
                        help='输出文件路径 (默认: stdlib/duanpub/__index__.py)')
    ...
    if args.output:
        ...
        # 相对当前工作目录
        output_path = Path('stdlib/duanpub/__index__.py')

    print(f"扫描 duanpub 包: {args.duanpub_path}/packages/")
    packages = scan_packages(args.duanpub_path)
    ...
    # 生成索引
    content = generate_index(packages, args.duanpub_path)
