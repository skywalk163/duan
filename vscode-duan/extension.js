...
function activate(context) {
    ...
    context.subscriptions.push(
        vscode.debug.registerDebugAdapterDescriptorFactory('duan', debugAdapterFactory)
    );
...
"`;
    ...
    // 编译当前文件
    const compileCmd = vscode.commands.registerCommand('duan.compileFile', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('请先打开一个 .duan 文件');
            return;
        ...
        const outputChannel = vscode.window.createOutputChannel('段言 编译');
        outputChannel.show(true);
...
"`;
    ...
    // 编译为原生可执行文件
    const compileNativeCmd = vscode.commands.registerCommand('duan.compileNative', async () => {
        vscode.window.showInformationMessage('原生编译需要 LLVM 工具链支持，请先安装 LLVM');
    });

    // 构建整个项目
    const buildProjectCmd = vscode.commands.registerCommand('duan.buildProject', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('请先打开项目中的 .duan 文件');
            return;
        }
        ...
        const outputChannel = vscode.window.createOutputChannel('段言 构建');
        outputChannel.show(true);
        ...
        try {
            ...
            // 收集所有 .duan 文件
            const duanFiles = findDuanFiles(projectRoot);
            outputChannel.appendLine(`找到 ${duanFiles.length} 个 .duan 文件\n`);

            for (const duanFile of duanFiles) {
                const relPath = path.relative(projectRoot, duanFile);
...
with open(r'${duanFile}', 'r', encoding='utf-8') as f:
    code = f.read()
...
"`;
    ...
    // 格式化当前文件
    const formatCmd = vscode.commands.registerCommand('duan.formatFile', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('请先打开一个 .duan 文件');
            return;
        ...
        if (formatted !== text) {
            ...
            vscode.window.showInformationMessage('段言文件已格式化');
        }
    });

    // 显示版本信息
    const versionCmd = vscode.commands.registerCommand('duan.showVersion', () => {
        const outputChannel = vscode.window.createOutputChannel('段言 版本信息');
        outputChannel.show(true);
        outputChannel.appendLine('=== 段言 (Duan) 语言 ===');
        ...
        outputChannel.appendLine('');
        outputChannel.appendLine('项目仓库: https://github.com/duan-lang/duan');
        outputChannel.appendLine('文档: https://github.com/duan-lang/duan/blob/main/docs/');
    });
    ...
    statusBarItem.text = '$(symbol-namespace) 段言 L2';
    statusBarItem.tooltip = '段言文体模式 (L1 白话 / L2 文言)';
    statusBarItem.command = 'duan.showVersion';
    statusBarItem.show();

    // 更新状态栏基于配置
    function updateStatusBar() {
        const style = vscode.workspace.getConfiguration('duan').get('style', 'L2');
        statusBarItem.text = `$(symbol-namespace) 段言 ${style}`;
    }
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('duan.style')) {
                updateStatusBar();
            }
        })
    );
...
function startLSPClient(context) {
    try {
        const pythonPath = vscode.workspace.getConfiguration('duan').get('pythonPath', 'python');
        const lspScript = path.join(context.extensionPath, '..', 'lsp', 'duan_lsp_main.py');

        if (!fs.existsSync(lspScript)) {
            console.log('段言 LSP 脚本未找到，跳过 LSP 客户端启动');
            return null;
        }
        ...
        console.log('段言 LSP 客户端已启动');
        return client;
    } catch (e) {
        console.log('段言 LSP 客户端启动失败（可选功能）:', e.message);
        return null;
    }
}
...
function createDebugAdapterFactory() {
    const factory = {
        createDebugAdapterDescriptor(session) {
            const pythonPath = vscode.workspace.getConfiguration('duan').get('pythonPath', 'python');
            const debugScript = path.join(__dirname, 'debug-adapter', 'duan_debug.py');
 ...
 * 查找所有 .duan 文件
 */
function findDuanFiles(rootDir) {
    const results = [];
    function walk(dir) {
        try {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'node_modules') {
                    walk(fullPath);
                } else if (entry.isFile() && entry.name.endsWith('.duan')) {
