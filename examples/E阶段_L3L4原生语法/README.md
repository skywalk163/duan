...
2. **L4 作用域隔离**：所有 `引 Python` 内部的中间变量（`import numpy as np`、`_tmp`、内部类…）现在都在独立的命名空间里执行，不会污染段言主作用域，只导出公共函数/数据。
...
**code_generator 自动生成 3 个段言函数**（`CREATE/INSERT` 走 DML，`SELECT` 走查询）：
...
- 未来（v4.1 E+）：加 `引 PG 业务库 / 引 MySQL 主库` 路由到 asyncpg / pymysql，段言调用侧写法不变

demo：[E1_L3_SQL原生.duan](./E1_L3_SQL原生.duan)
...
也就是你不用再写 `d["年"]` 这种「字典写法」，直接 `.年` 就是**段言 L0 风格的成员访问**。

demo：[E2_L3_模式正则原生.duan](./E2_L3_模式正则原生.duan)
...
code_generator 按行解析，每行生成一个段言函数。未安装 `sympy` 时**会在调用时抛出友好异常**（`sympy未装: ...`），不会在加载时崩溃。

demo：[E3_L3_公式数学原生.duan](./E3_L3_公式数学原生.duan)

---

## 4. L4 作用域隔离：引 Python 不再"污染"段言变量

**之前（v4.0 D 及更早）**：引 Python 里写的代码和段言生成的代码共享同一个 Python 模块命名空间。
```python
# 引 Python:
import numpy as np
_tmp = 9999
def f(x): return x*x
```
→ 段言里你会有一个叫 `np` 的变量、一个叫 `_tmp` 的变量。万一你自己也想叫 `np` 就重名了；更糟的是内部类、密码、配置也会泄漏。

**现在（v4.0 E）**：引 Python 里的代码都在一个独立的 `types.ModuleType("_DUAN_L4_NS")` 里执行，然后按**三条导出规则**决定哪些名字暴露给段言：
...
| `import numpy as np` | ❌ | `np` 是 module，不在白名单（保护段言侧） |
...
demo：[E4_L4_沙箱隔离验证.duan](./E4_L4_沙箱隔离验证.duan)
