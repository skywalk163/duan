...
def compile_duan(source):
    """编译段言源代码，返回生成的 Python 代码"""
...
def benchmark_duan_vs_python():
    """段言代码执行时间与等效 Python 代码对比"""
    print("=" * 80)
    print("1. 段言 vs Python 运行时性能对比")
    print("=" * 80)
    ...
    for case in BENCHMARK_CASES:
        ...
        for _ in range(3):
            compiler = DuanCompiler()
            _, t_compile = time_it(compiler.compile, case['duan'])
            code = compiler.compile(case['duan'])
            duan_compile_time += t_compile

            _, t_exec = time_it(exec, code, {})
            duan_exec_times.append(t_exec)
        duan_compile_avg = duan_compile_time / 3
        duan_exec_avg = sum(duan_exec_times) / len(duan_exec_times)

        print(f"    段言: 编译={format_time(duan_compile_avg)}, 执行={format_time(duan_exec_avg)}")
        ...
        # 对比
        ratio = duan_exec_avg / py_exec_avg if py_exec_avg > 0 else float('inf')
        print(f"    对比: 段言/Python = {ratio:.2f}x")

        result = {
            'name': name,
            'duan_compile_avg': duan_compile_avg,
            'duan_exec_avg': duan_exec_avg,
            'python_exec_avg': py_exec_avg,
            'ratio_duan_to_python': ratio,
        }
...
'''
    try:
        compiler = DuanCompiler()
        code = compiler.compile(duan_math_code)
        _, t_duan_math = time_it(exec, code, {})
        print(f"    段言数学运算: {format_time(t_duan_math)}")
        math_results['tests'].append({'name': '数学运算', 'duan_time': t_duan_math})
    ...
    math_results['tests'][-1]['ratio'] = t_duan_math / t_py_math if t_py_math > 0 else float('inf')
...
'''
    try:
        compiler = DuanCompiler()
        code = compiler.compile(duan_str_code)
        _, t_duan_str = time_it(exec, code, {})
        print(f"    段言字符串操作: {format_time(t_duan_str)}")
        string_results['tests'].append({'name': '字符串拼接', 'duan_time': t_duan_str})
    except Exception as e:
        print(f"    段言字符串操作: ✗ 错误: {e}")
    ...
    if 'duan_time' in string_results['tests'][-1]:
        string_results['tests'][-1]['python_time'] = t_py_str
        string_results['tests'][-1]['ratio'] = string_results['tests'][-1]['duan_time'] / t_py_str if t_py_str > 0 else float('inf')
...
'''
    try:
        compiler = DuanCompiler()
        code = compiler.compile(duan_sort_code)
        _, t_duan_sort = time_it(exec, code, {})
        print(f"    段言冒泡排序(200元素): {format_time(t_duan_sort)}")
        sort_results['tests'].append({'name': '冒泡排序(200)', 'duan_time': t_duan_sort})
    except Exception as e:
        print(f"    段言冒泡排序: ✗ 错误: {e}")
    ...
    if 'duan_time' in sort_results['tests'][-1]:
        sort_results['tests'][-1]['python_time'] = t_py_sort
        sort_results['tests'][-1]['ratio'] = sort_results['tests'][-1]['duan_time'] / t_py_sort if t_py_sort > 0 else float('inf')
...
'''
    try:
        compiler = DuanCompiler()
        code = compiler.compile(duan_list_code)
        _, t_duan_json = time_it(exec, code, {})
        print(f"    段言列表数据处理(500条): {format_time(t_duan_json)}")
        json_results['tests'].append({'name': '列表数据处理(500)', 'duan_time': t_duan_json})
    except Exception as e:
        print(f"    段言列表数据处理: ✗ 错误: {e}")
    ...
    if 'duan_time' in json_results['tests'][-1]:
        json_results['tests'][-1]['python_time'] = t_py_json
        json_results['tests'][-1]['ratio'] = json_results['tests'][-1]['duan_time'] / t_py_json if t_py_json > 0 else float('inf')
...
def generate_report(duan_vs_py_results, stdlib_results, output_path):
    """生成结构化 JSON 报告"""
    report = {
        'report_type': 'runtime_benchmark',
        'timestamp': datetime.now().isoformat(),
        'duan_vs_python': duan_vs_py_results,
        'stdlib_benchmark': stdlib_results,
        'summary': {},
    }

    # 计算汇总
    if duan_vs_py_results:
        avg_ratio = sum(r.get('ratio_duan_to_python', 0) for r in duan_vs_py_results) / len(duan_vs_py_results)
        report['summary'] = {
            'test_count': len(duan_vs_py_results),
            'avg_duan_to_python_ratio': avg_ratio,
            'fastest_relative': min(duan_vs_py_results, key=lambda r: r.get('ratio_duan_to_python', float('inf')))['name'],
            'slowest_relative': max(duan_vs_py_results, key=lambda r: r.get('ratio_duan_to_python', 0))['name'],
        }
...
def main():
    parser = argparse.ArgumentParser(description='段言编译器运行时性能基准测试')
    parser.add_argument('--output', '-o', default=str(REPORT_DIR / 'runtime_benchmark.json'),
                        help='JSON 报告输出路径')
    parser.add_argument('--no-compare', action='store_true', help='跳过段言 vs Python 对比')
    parser.add_argument('--no-stdlib', action='store_true', help='跳过标准库测试')
    args = parser.parse_args()

    print("段言编译器 - 运行时性能基准测试")
    print("=" * 80)
    print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Python: {sys.version.split()[0]}")
    print()

    duan_vs_py_results = None
    stdlib_results = None

    if not args.no_compare:
        duan_vs_py_results = benchmark_duan_vs_python()

    if not args.no_stdlib:
        stdlib_results = benchmark_stdlib_math()

    # 生成报告
    report = generate_report(duan_vs_py_results, stdlib_results, args.output)
