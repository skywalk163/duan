"""
verify_bootstrap.py - 验证段言自举编译器的二次自举一致性

流程：
1. SRC 后端编译 bootstrap_v3.duan → bootstrap_v3_gen.py（第一次编译）
2. bootstrap_v3_gen.py 编译 bootstrap_v3.duan → bootstrap_v3_gen2.py（第二次编译）
...
def _setup_duan_builtin():
    ...
    _duan_builtin.最小值 = lambda *args: min(args) if len(args) > 1 else min(args[0])
    _duan_builtin.最大值 = lambda *args: max(args) if len(args) > 1 else max(args[0])
    ...
    _duan_builtin.格式化 = lambda fmt, *args: fmt.format(*args)
    _duan_builtin.映射 = lambda func, lst: list(map(func, lst))
    _duan_builtin.过滤 = lambda func, lst: list(filter(func, lst))
    _duan_builtin.归并 = lambda func, lst, initial=None: (
...
def step1_src_compile():
    """第一步：用 SRC 后端编译 bootstrap_v3.duan → bootstrap_v3_gen.py"""
    print("=" * 70)
    print("第一步：SRC 后端编译 bootstrap_v3.duan → bootstrap_v3_gen.py")
    print("=" * 70)
    
    from duan_parser_v3 import DuanParser
    from code_generator import PythonCodeGenerator
    
    bootstrap_path = os.path.join(_script_dir, 'bootstrap', 'bootstrap_v3.duan')
...
def step2_self_compile():
    """第二步：用 bootstrap_v3_gen.py 编译 bootstrap_v3.duan → bootstrap_v3_gen2.py"""
    print()
    print("=" * 70)
    print("第二步：自举编译 bootstrap_v3.duan → bootstrap_v3_gen2.py")
    print("=" * 70)
...
'''
    ...
    try:
        ...
        # 执行测试程序
        print("\n执行测试程序:")
        test_ns = {'_duan_builtin': _duan_builtin}
