...
Environment: G:\dumategithub\duan (段言/Duan programming language)
Date: 2026-07-05
...
Running `duan run hello.duan` with src backend produces:
...
everywhere — including at module level — which is valid Duan syntax (a script
...
┌──────────────────────────────────────────────────────────────────────────────┐
│  Duan AST: Module                              Python: invalid                 │
│  ├── VarDecl(甲 = 3)                         甲 = 3                            │
...
Affected files:
    • src/code_generator.py            (primary — used by `duan run`)
    ...
    Wrap the generated `return` in a conditional:
        ...
        def _generate_return_stmt(self, stmt: ReturnStmt):
            if self._in_function:
                ...
                if stmt.value:
                    value = self._generate_expr(stmt.value)
                    self._add_line(f"_duan_result = {value}")
...
After fix, the reproduction script should produce:
    ...
    → Generated code: "甲 = 3" (line N)
                      "_duan_result = 甲"  (line N+1)   ← no SyntaxError
    → exec(): SUCCESS
...
• The `_duan_result` variable in the module-level case is harmless; it allows
...
• If stricter behaviour is wanted, the module-level return value could also
  ...
  Using `_duan_result = 甲` is the safest compromise.
