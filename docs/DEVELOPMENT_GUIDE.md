...
欢迎阅读段言开发指南！本文档面向所有希望参与段言开发的贡献者，从环境搭建到代码提交流程，从编码规范到测试要求，提供全面的指导。
...
段言采用 **基于分支的开发流程**，每个功能或修复在独立分支上开发，经审查后合并到主分支。
...
1. 在 [GitHub Issues](https://github.com/skywalk163/duan/issues) 中找到你想解决的问题
...
| 类别 | 规范 | 示例 |
|------|------|------|
| 文件名 | 小写加下划线 | `duan_ast.py`、`code_generator.py` |
| 类名 | 大驼峰（PascalCase） | `DuanVisitor`、`AstAdapter`、`LLVMCodeGen` |
...
段言项目遵循 **Conventional Commits** 规范：
...
如果你是第一次参与段言开发，以下任务类型是不错的起点：

| 任务类型 | 难度 | 所需技能 | 预计时间 |
|----------|------|----------|----------|
| 文档改进 | ⭐ | Markdown 写作 | 1-2 小时 |
| Bug 报告 | ⭐ | 使用段言的经验 | 30 分钟 |
...
**Q: 我不熟悉编译器开发，能参与吗？**
A: 当然可以！段言项目有很多非编译器核心的贡献机会，如文档、测试、示例、工具链等。
